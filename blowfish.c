...
void Blowfish_Encrypt(BLOWFISH_CTX *ctx, uint32_t *xl, uint32_t *xr) {
    ...
    Xr ^= ctx->P[N];     // Final XOR with last P-array value
    Xl ^= ctx->P[N + 1];
...
void Blowfish_Decrypt(BLOWFISH_CTX *ctx, uint32_t *xl, uint32_t *xr) {
    ...
    Xr ^= ctx->P[1];  // Reverse initial XOR
    Xl ^= ctx->P[0];
...
void Blowfish_Encrypt_CBC(BLOWFISH_CTX *ctx, uint8_t *data, uint8_t *iv, size_t length) {
    ...
    memcpy(prev_block, iv, 8); // Start with IV
    ...
    for (size_t i = 0; i < length; i += 8) {
        ...
        Blowfish_Encrypt(ctx, (uint32_t *)(data + i), (uint32_t *)(data + i + 4));

        memcpy(prev_block, data + i, 8); // Update for next round
    }
}
...
void Blowfish_Decrypt_CBC(BLOWFISH_CTX *ctx, uint8_t *data, uint8_t *iv, size_t length) {
    ...
    for (size_t i = 0; i < length; i += 8) {
        memcpy(temp, data + i, 8);  // Save ciphertext before decryption

        Blowfish_Decrypt(ctx, (uint32_t *)(data + i), (uint32_t *)(data + i + 4));

        // XOR decrypted block with previous ciphertext block
        for (int j = 0; j < 8; j++)
            data[i + j] ^= prev_block[j];

        memcpy(prev_block, temp, 8); // Update for next round
    }
}
